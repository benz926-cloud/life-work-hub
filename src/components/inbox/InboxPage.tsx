...
import { useInboxData } from "@/hooks/useData";
...
export default function InboxPage() {
  const { items, create, update, error, refresh, isDemo } = useInboxData();
  ...
  const handleSubmit = async () => {
    ...
    const item = await create(toInboxItem(result.data, ""));
    if (item) setActionsById((current) => ({ ...current, [item.id]: result.data.suggestedActions.slice(0, 3) }));
    setInput("");
  };
  ...
  return <div className="space-y-4"><SectionHeader title="AI 收件箱" subtitle="说出任何想法，先看 AI 如何理解，再决定提交。" /><DemoBanner isDemo={isDemo} /><DataError error={error} onRetry={() => void refresh()} />
    ...
    {filteredItems.length > 0 ? <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">{filteredItems.map((item) => { const config = categoryConfig[item.category]; const actions = actionsById[item.id] ?? []; return <Card key={item.id}><div className="p-4"><div className="mb-2 flex items-center gap-2"><span>{config.icon}</span><span className="text-xs font-medium text-slate-500">{config.label}</span>{item.priority === "high" && <Badge variant="danger">紧急</Badge>}</div><div className="mb-3 text-sm leading-relaxed text-slate-900 dark:text-white">{item.content}</div>{actions.length > 0 && <div className="mb-3 space-y-1.5 rounded-xl bg-slate-50 p-2.5 dark:bg-slate-800/80"><div className="flex items-center gap-1 text-[11px] font-medium text-slate-500"><Tag size={12} />建议下一步</div>{actions.map((action, index) => <button key={`${item.id}-${index}`} className="block text-left text-xs text-sky-700 hover:underline dark:text-sky-300">{action}</button>)}</div>}{item.ai_result && <div className="mb-3 rounded-lg bg-slate-50 p-2 text-xs text-slate-500 dark:bg-slate-800"><span className="font-medium text-emerald-600">AI 已完成处理</span></div>}<div className="flex items-center justify-between"><span className="flex items-center gap-1 text-xs text-slate-400"><CalendarDays size={12} />{new Date(item.created_at).toLocaleDateString("zh-CN")}</span><div className="flex gap-1">{item.status === "pending" && <button onClick={() => void update(item.id, { status: "completed" })} className="rounded bg-emerald-100 px-2 py-1 text-xs text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-400">完成</button>}<button onClick={() => void update(item.id, { status: "archived" })} className="rounded bg-slate-100 px-2 py-1 text-xs text-slate-600 dark:bg-slate-800 dark:text-slate-400">归档</button></div></div></div></Card>; })}</div> : <EmptyState title="这个分类还没有内容，写下一条想法开始吧。" />}</div>;
}
